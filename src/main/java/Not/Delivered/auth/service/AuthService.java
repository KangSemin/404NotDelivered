...
public class AuthService {
  ...
  public User signUpUser(SignupRequestDto request) {
    ...
    User user = User.builder().email(request.getEmail()).password(encodedPassword)
        .userName(request.getUsername()).userStatus(request.getUserStatus())
  ...
  public User login(LoginRequestDto request) {
    ...
    if (user.getIsWithdrawal()) {
      throw new TempUserNotFoundException("이미 탈퇴한 유저입니다.");
    }
